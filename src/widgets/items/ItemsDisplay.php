...
namespace common\modules\catalogs\widgets\items;
...
use common\modules\catalogs\models\ItemSearch;
 ...
 */
class ItemsDisplay extends DisplayWidget
{
    /**
     * {@inheritdoc}
     */
    ...
    public function getDataProvider()
    {
        if (!$this->dataProvider instanceof DataProviderInterface) {
            ...
            $this->dataProvider = (new ItemSearch())->search($params, $condition);
        }
    ...
    public function getDeatilUrl($model)
    {
        if (!is_callable($this->detailUrl)) {
            $this->detailUrl = function ($model) {
                return Framework::composeUrl(Module::class, 'items/view', ['id' => $model->id]);
            };
        }
